...
import { cacheGet, cacheSet, cacheBackend } from './cache.js';
import { classifyStory, isStoppedOrParked } from './classify.js';
import { FEATURED_STORIES } from './featuredStories.js';

const HERMAI_URL = 'https://api.hermai.ai/v1/fetch';
const CACHE_KEY = 'crashtracker:crashes';

// The real dataset updates monthly at the source. This TTL just avoids
...
export async function fetchCrashes({ force = false } = {}) {
  ...
  const response = await fetch(HERMAI_URL, {
    ...
    body: JSON.stringify({ site, endpoint, params: {} })
  });
  ...
  const unmatchedCities = [...new Set(
    crashes.filter(c => c.lat == null).map(c => c.city)
  )].sort();

  if (unmatchedCities.length) {
    console.warn(
      `${unmatchedCities.length} crash(es) have no geocode match and won't show on the map:`,
      unmatchedCities
    );
  }

  const entry = { data: crashes, fetchedAt: Date.now(), unmatchedCities };
  await cacheSet(CACHE_KEY, entry, CACHE_TTL_SECONDS);
  return crashes;
}
...
export async function getCacheMeta() {
  ...
  return {
    ...
    count: cached ? cached.data.length : 0,
    backend: cacheBackend(),
    unmatchedCities: cached ? cached.unmatchedCities || [] : []
  };
}
...
function normalizeCrash(row) {
  ...
  const id = row['Report ID'] || row['SGO Report ID'] || randomId();
  const story = classifyStory({ severity, narrative });

  // Belt-and-suspenders: even though FEATURED_STORIES is a hand-checked
  // allowlist, never surface one in the "you'd never guess" rotation if
  // this specific fetch's own data says it's serious — e.g. if a future
  // monthly refresh ever changes what's on file for that report ID.
  const featuredHeadline = (FEATURED_STORIES[id] && story !== 'serious')
    ? FEATURED_STORIES[id]
    : null;
  ...
  return {
    ...
    severity,
    story, // 'serious' | 'standard'
    featuredHeadline, // non-null only for the exact allowlisted report IDs
    stoppedOrParked: isStoppedOrParked(movement),
